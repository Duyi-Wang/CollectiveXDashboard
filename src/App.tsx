...
export default function App() {
  ...
  const kvChart = useMemo(
    ...
    [datasets, kvView, kvOp, effectivePage, effectiveIsl],
  );
  ...
  const yLabel =
    tab === "kv"
      ? {
          ...
          latency: "Burst latency p50 (ms)",
          frontier: "Best bandwidth at each ISL (GB/s)",
          overlap: "Bandwidth / batch-1 bandwidth (×)",
        }[kvView]
  ...
  async function exportChart(format: "svg" | "png") {
    ...
    if (!svg) return;
    try {
      if (format === "svg") exportSvg(svg, "collectivex-chart.svg", shownChart);
      else await exportPng(svg, "collectivex-chart.png", shownChart);
    } catch (e) {
      setMessages((m) => [
        ...m,
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        <a href="./" className="brand" aria-label="CollectiveX 首页">
      ...
      <main>
        <div className="page-heading">
          <div>
            ...
            <h1>
              看清每一次 GPU 通信<span className="heading-dot">.</span>
            </h1>
            <p className="subtitle">
              对比 CollectiveX
              延迟与带宽，探索不同硬件、通信库和并行配置的性能边界。
            </p>
          </div>
        ...
        <div className="source-strip">
          <span className="strip-label">连接数据源</span>
          <button onClick={() => setSource("local")}>
            <Upload size={15} />
            本地文件<span>JSON / CSV / ZIP</span>
          </button>
          <i />
          ...
          <button onClick={() => setSource("official")}>
            <Database size={15} />
            InferenceX<span>官方数据库</span>
          </button>
          <span className="strip-end">工作区自动保存在当前浏览器</span>
        </div>
        {messages.length > 0 && (
          <div className="message-banner" role="status">
            <div>
              ...
              {messages.length > 8 && (
                <p>另有 {messages.length - 8} 条格式提示。</p>
              )}
            </div>
            <button
              className="icon-button"
        ...
        <section className="stat-grid" aria-label="当前选择的数据概览">
          <div className="stat-card">
            <span>
              已选运行 <Layers3 size={16} />
            </span>
            <strong>
          ...
          <div className="stat-card">
            <span>
              硬件平台 <Network size={16} />
            </span>
            <strong>{skus.length.toString().padStart(2, "0")}</strong>
            <p title={skus.join(" · ")}>
              {skus.slice(0, 4).join(" · ") || "等待选择运行"}
            </p>
          </div>
          <div className="stat-card">
            <span>
              实测数据点 <Activity size={16} />
            </span>
            <strong>{number(measured, 0)}</strong>
            <p>从已载入的 EP / KV 数据逐点计数</p>
          </div>
          <div className="stat-card accent-card">
            <span>
              数据来源 <Database size={16} />
            </span>
        ...
        <section className="panel run-panel">
          <div className="panel-heading">
            <div>
              <h2>
                运行工作区 <span className="count">{datasets.length}</span>
              </h2>
              <p>勾选运行叠加比较。颜色标识配置，线型区分运行。</p>
            </div>
          ...
          {loading ? (
            ...
            <div className="empty">
              <Database size={28} />
              <h3>添加第一组测试数据</h3>
              <p>导入本地文件、CI run 或官方数据开始分析。</p>
              <button onClick={() => setSource("local")}>添加数据</button>
            </div>
          ) : (
                    ...
                    <tr key={d.id} className={d.visible ? "selected-row" : ""}>
                      <td>
                        <input
                          type="checkbox"
                          checked={d.visible}
                          aria-label={`显示 ${originLabel[d.origin]} ${d.dataset.run.run_id}`}
                          onChange={(e) =>
                            setDatasets((old) =>
                              old.map((v) =>
                                v.id === d.id
                      ...
                      <td>
                        <div className="run-id">
                          ...
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            aria-label={`打开 run ${d.dataset.run.run_id}`}
                          >
                      ...
                      <td className="mono">
                        {shortDate(d.dataset.run.generated_at)}
                      </td>
                      <td>
                      ...
                      <td>
                        <button
                          className="icon-button"
                          aria-label={`移除 ${d.dataset.run.run_id}`}
                          onClick={() =>
                            setDatasets((old) =>
        ...
        <div className="explorer-tabs" role="tablist" aria-label="分析视图">
          {(
            [
          ...
          <span className="tab-note">CollectiveX v1 · 可追溯到原始运行</span>
        </div>
        {tab !== "coverage" ? (
          <section className="panel explorer-panel">
            <div className="filter-toolbar">
              ...
              {tab === "ep" ? (
                <>
                  {(
                    [
                      ["phase", "阶段", allSeries.map((s) => s.phase)],
                      ["mode", "Kernel 模式", allSeries.map((s) => s.mode)],
                      ["precision", "精度", allSeries.map((s) => s.precision)],
                      [
                        "ep",
                        "EP",
                        allSeries.map((s) => String(s.system.ep_size)),
                      ],
                      ["sku", "硬件 SKU", allSeries.map((s) => s.system.sku)],
                      ["backend", "通信库", allSeries.map((s) => s.backend)],
                ...
                <>
                  ...
                  {kvView !== "frontier" && (
                    <Select
                      label="ISL"
                      value={effectiveIsl}
                      options={[["max", "各 case 最大 ISL"], ...kvIsls]}
                      onChange={setKvIsl}
                    />
                  )}
                </>
            ...
            <div className="chart-heading">
              <div>
                ...
                <h2>{tab === "ep" ? "通信性能曲线" : "KV Cache 传输性能"}</h2>
                <p>
                  {tab === "ep"
                    ? METRICS[metric].hint
                    : kvView === "overlap"
            ...
            <div className="chart-controls">
              {tab === "ep" && (
                <>
                  <div className="segmented" aria-label="性能指标">
            ...
            <div className="chart-layout">
              ...
              <aside className="legend">
                <div className="legend-heading">
                  <strong>
                    对比系列 <span>{allChart.length}</span>
                  </strong>
                  <button onClick={() => setHidden(new Set())}>显示全部</button>
                </div>
                <p>点击显示 / 隐藏曲线</p>
                  ...
                  {allChart.map((s) => (
                    ...
                    >
                      ...
                      <span>
                        {s.label}
                        <small>
                          {s.points.filter((p) => p.y != null).length} 个测量点
                        </small>
                      </span>
                    </button>
                  ))}
            ...
            <div className="chart-footer">
              <span>
                ...
                {shownChart.length} 条可见系列 · {plotPoints.length} 个测量点
              </span>
              <span>悬停或聚焦数据点查看详细测量值</span>
            </div>
            {tab === "ep" && (
              <details className="methodology">
                <summary>测量口径与带宽拟合</summary>
                <p>
                  延迟单位为 µs；带宽使用十进制 GB/s。Payload 带宽按 GPU
                  归一，包含 FP8 scale bytes；Activation 带宽是所有 rank
                  的聚合值。p99 速率表示“p99 延迟所对应的速率”，不是速率分布的
                  p99。
                </p>
                <p>
                  只对同一配置的原始 payload bytes / EP
                  与延迟做最小二乘拟合：latency ≈ α + bytes / β。至少需要 3
                  个有效点与正斜率；拟合只作诊断，不作为硬件线速结论。
                </p>
                <div className="table-scroll">
                  <table>
                    <thead>
                        ...
                        .map(({ series: s, owner, id }) => {
                          ...
                          return (
                            <tr key={id}>
                              ...
                              <td>
                                {fit
                                  ? `${fit.betaReliable ? "拟合可参考" : "β 不可靠（仅诊断）"}${fit.alphaExtrapolated ? " · α 为远端外推" : ""}`
                                  : "有效点不足或斜率非正"}
                              </td>
                            </tr>
                          );
          ...
          <section className="panel coverage-panel">
            ...
            <div className="table-scroll">
                    ...
                    .map((c, i) => (
                      <tr key={`${c.runId}:${c.case_id}:${i}`}>
                        ...
                        <td>
                          {c.measured} / {c.total}
                          {c.suite === "KV" && (
                            <small className="table-sub">仅含已返回行</small>
                          )}
                        </td>
              ...
              {!filteredCoverage.length && (
                <div className="empty">没有符合条件的测试记录</div>
              )}
            </div>
            <div className="pagination">
              <span>
                {filteredCoverage.length} 条记录 · 第 {tablePage + 1} 页
              </span>
              <div>
                <button
                  disabled={!tablePage}
          ...
          <section className="panel raw-panel">
            ...
            >
              <span>
                <Activity size={17} />
                <strong>图表数据</strong>
                <small>{plotPoints.length} 个可见点</small>
              </span>
              <ChevronDown
                size={17}
        ...
        <footer>
          <span>
            CollectiveX Dashboard <i /> 基于 InferenceX
            公开数据与测试协议独立构建
          </span>
          <a
            ...
            rel="noreferrer"
          >
            研究起点 · PR #2004 <ArrowUpRight size={13} />
          </a>
        </footer>
      </main>
      {source && (
