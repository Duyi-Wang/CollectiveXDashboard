...
2026-09-11：标题统一为用户指定的 `CollectiveX · Mori Internal dashboard`，中英文界面保留同一产品名称。水印字体从 54 增至 120，沿接近整条绘图对角线展开；实际边界宽度约 854 / 884（96.7%），且全部处于绘图区内，未侵入坐标轴。SVG/PNG 像素验证继续通过，完整 64 项单元测试、14 项浏览器测试及构建通过；390px 页面没有横向溢出。README 仍不嵌入图片，测试预览服务已关闭。
